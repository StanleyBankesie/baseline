...
export default function CustomerOutstandingReportPage({ backPath = "/finance", backLabel = "Back to Finance" }) {
  ...
  // Account filter state - searchable dropdown
  const [accounts, setAccounts] = useState([]);
  const [accountId, setAccountId] = useState("");
  const [accountQuery, setAccountQuery] = useState("");
  const [showAccountDropdown, setShowAccountDropdown] = useState(false);

  // Load debtors accounts
  async function loadAccounts() {
    try {
      ...
      // Filter for asset nature accounts (debtors) by default
      const debtorsAccounts = allAccounts.filter(
        (a) => String(a.nature || a.group_nature || "").toUpperCase() === "ASSET"
      );
      setAccounts(debtorsAccounts);
    } catch {
  ...
  const filteredAccountOptions = useMemo(() => {
    ...
    return filterAndSort(accountOptions, {
      query: q,
      getKeys: (o) => [o.label, o.code, o.name],
    }).slice(0, 10);
  }, [accountQuery, accountOptions]);

  // Selected account display
  const selectedAccount = useMemo(() => {
    return accountOptions.find((a) => String(a.value) === String(accountId));
  ...
  async function run() {
    try {
      ...
      toast.error(e?.response?.data?.message || "Failed to load customer outstanding");
    } finally {
      setLoading(false);
    }
  }
  ...
  const filteredDetail = items.filter((r) =>
    !customerQuery || String(r.customer_name || "").toLowerCase().includes(customerQuery.toLowerCase())
  );
  ...
  function exportExcel() {
    ...
    const agingData = [
      ...
      [`As at: ${asOfDate}`, "", "", "", "", "", ""],
      [],
      ["Customer", "Current (≤0d)", "1-30 Days", "31-60 Days", "61-90 Days", ">90 Days", "Total Outstanding"],
      ...filteredSummary.map((s) => [
        s.customer_name, s.current, s["1_30"], s["31_60"], s["61_90"], s.over_90, s.total,
      ]),
      [],
    ...
    // Detail Sheet
    const detailData = [
      ["Customer", "Invoice No", "Invoice Date", "Due Date", "Total", "Received", "Outstanding", "Days Overdue", "Bucket"],
      ...filteredDetail.map((r) => [
        r.customer_name, r.invoice_no, fmtDate(r.invoice_date), fmtDate(r.due_date),
        Number(r.amount || 0), Number(r.received || 0), Number(r.outstanding || 0),
    ...
    XLSX.writeFile(wb, `customer-outstanding-${asOfDate}.xlsx`);
  }
  ...
  function exportPDF() {
    ...
    doc.text("CUSTOMER OUTSTANDING ANALYSIS", pageW / 2, 11, { align: "center" });
    doc.setFontSize(9);
    ...
    doc.setFontSize(8);
    const cols = [
      { label: "Customer", x: margin + 1, w: 60 },
    ...
    filteredSummary.forEach((s, idx) => {
      ...
      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.text(String(s.customer_name || "").slice(0, 38), margin + 1, y);
    ...
    doc.save(`customer-outstanding-${asOfDate}.pdf`);
  }
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">
            Receivables aging analysis by customer as at {asOfDate}
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex flex-wrap items-center gap-4 bg-slate-50 dark:bg-slate-800/50">
          {/* Searchable Account (Debtors) Dropdown */}
          <div className="relative">
            <span className="text-sm font-bold text-slate-600 dark:text-slate-400 block mb-1">Account (Debtors)</span>
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (selectedAccount) setAccountId("");
                }}
                onFocus={() => setShowAccountDropdown(true)}
              />
              {showAccountDropdown && (
                <div className="absolute z-50 mt-1 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                  <div 
                    className="px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-100 dark:border-slate-700"
                    onClick={() => {
                      setAccountId("");
                      setAccountQuery("");
                  ...
                  >
                    <span className="font-medium text-slate-700 dark:text-slate-200">All Debtors</span>
                  </div>
                    ...
                    filteredAccountOptions.map((a) => (
                      <div
                        ...
                        className={`px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer ${
                          String(accountId) === a.value ? "bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300" : ""
                        }`}
                        onClick={() => {
                          setAccountId(a.value);
                          setAccountQuery("");
                          setShowAccountDropdown(false);
                        }}
                      >
                        <span className="font-medium text-slate-700 dark:text-slate-200">{a.label}</span>
                      </div>
                    ))
                  )}
                </div>
          ...
          <input className="input input-bordered input-sm w-52" placeholder="🔍 Search customer..."
          ...
          <div className="flex gap-2 ml-auto">
            ...
            <button className={`btn btn-sm ${view === "detail" ? "btn-primary" : "btn-ghost"}`}
              onClick={() => setView("detail")}>Invoice Detail</button>
          </div>
          ...
          {view === "aging" ? (
            <table className="table table-sm w-full text-sm">
              <thead className="bg-slate-800 text-white">
                <tr>
                  <th className="py-3 text-xs uppercase tracking-wider">Customer</th>
              ...
              <tbody>
                {filteredSummary.length === 0 && !loading && (
                  <tr><td colSpan={7} className="text-center py-10 text-slate-400">No outstanding receivables found</td></tr>
                )}
                {filteredSummary.map((s, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 border-b border-slate-100 dark:border-slate-700/50">
                    <td className="py-2.5">
                      <div className="font-semibold text-slate-700 dark:text-slate-200">{s.customer_name}</div>
                      {s.customer_code && <div className="text-xs text-slate-400">{s.customer_code}</div>}
                    </td>
            ...
            <table className="table table-sm w-full text-sm">
              <thead className="bg-slate-800 text-white">
                <tr>
                  <th className="py-3 text-xs uppercase">Customer</th>
                  <th className="py-3 text-xs uppercase">Invoice No</th>
                  <th className="py-3 text-xs uppercase">Invoice Date</th>
                  <th className="py-3 text-xs uppercase">Due Date</th>
                  <th className="text-right py-3 text-xs uppercase">Total</th>
                  <th className="text-right py-3 text-xs uppercase">Received</th>
              ...
              <tbody>
                ...
                {filteredDetail.map((r, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 border-b border-slate-100 dark:border-slate-700/50">
                    <td className="font-medium text-slate-700 dark:text-slate-200">{r.customer_name || "—"}</td>
                    <td className="font-mono text-brand text-xs">{r.invoice_no}</td>
                    <td className="text-xs text-slate-500">{fmtDate(r.invoice_date)}</td>
                    <td className="text-xs text-slate-500">{fmtDate(r.due_date)}</td>
                    <td className="text-right font-mono">₵{fmt(r.amount)}</td>
                    <td className="text-right font-mono text-emerald-600">₵{fmt(r.received)}</td>
