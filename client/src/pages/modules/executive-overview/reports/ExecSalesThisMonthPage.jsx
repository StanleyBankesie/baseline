...
export default function ExecSalesThisMonthPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      } catch (e) { toast.error("Failed to load monthly sales"); }
    ...
    <div className="space-y-6 p-4">
      <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 mt-2">📊 Total Sales — {monthName}</h1>
          <p className="text-slate-500 text-sm mt-1">Month-to-date sales performance</p>
        </div>
        <button className="btn btn-outline btn-sm" onClick={() => { const ws = XLSX.utils.json_to_sheet(items); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "MonthlySales"); XLSX.writeFile(wb, "monthly-sales.xlsx"); }} disabled={!items.length}>📊 Excel</button>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "MTD Sales", value: cards.mtd_sales, color: "from-violet-500 to-violet-600" },
          { label: "MTD Gross Profit", value: cards.mtd_gross_profit, color: "from-emerald-500 to-emerald-600" },
          { label: "YTD Sales", value: cards.ytd_sales, color: "from-blue-500 to-blue-600" },
          { label: "YTD Gross Profit", value: cards.ytd_gross_profit, color: "from-indigo-500 to-indigo-600" },
        ].map((c, i) => (
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
          <h3 className="font-bold text-slate-700 dark:text-slate-200">Top Products This Month</h3>
        </div>
          ...
          <table className="table table-sm w-full text-sm">
            <thead className="bg-violet-700 text-white">
              <tr>
                ...
                <th className="text-right py-3 text-xs uppercase">MTD Value</th>
              </tr>
            </thead>
            <tbody>
              {items.length === 0 && !loading && <tr><td colSpan={3} className="text-center py-10 text-slate-400">No sales this month</td></tr>}
              {items.map((r, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                  ...
                  <td className="text-right font-black font-mono text-violet-700 dark:text-violet-400">₵{fmt(r.value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
