            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-brand-600 bg-brand-50 border-b border-brand-200">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold uppercase">Destination</th>
                  <th className="px-6 py-4 font-semibold uppercase">Cargo</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  requests.map((r) => (
                    ...
                    >
                      <td className="px-6 py-4 font-medium">{r.request_number}</td>
                      <td className="px-6 py-4">{r.request_date ? r.request_date.split('T')[0] : ''}</td>
                      <td className="px-6 py-4">{r.origin}</td>
                      <td className="px-6 py-4">{r.destination}</td>
                      <td className="px-6 py-4 max-w-[200px] truncate">{r.cargo_description}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          r.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' :
                          r.status === 'SCHEDULED' ? 'bg-blue-100 text-blue-800' :
                          r.status === 'COMPLETED' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'
                        }`}>
                          {r.status || 'PENDING'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Link to={`/transport/requests/${r.id}`} className="btn btn-ghost btn-sm text-brand-600">
                            <EditOutlined />
                          </Link>
                        </div>
                      </td>
