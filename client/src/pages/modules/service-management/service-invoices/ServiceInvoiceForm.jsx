...
export default function InvoiceForm() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const usedSoIds = new Set(
        invoices
          .map((inv) => inv?.service_execution_id)
          .filter((v) => v !== undefined && v !== null)
          .map((v) => String(v)),
      );
      let filtered = baseOrders.filter((o) => o.status === "POSTED" && !usedSoIds.has(String(o.id)));
      if (isEdit && form.service_execution_id) {
        const sid = String(form.service_execution_id);
        const exists = filtered.some((o) => String(o.id) === sid);
        if (!exists) {
          try {
            const res = await api.get(`/purchase/service-executions/${form.service_execution_id}`);
    ...
    try {
      ...
      if (data) {
        setForm({
          ...
          service_execution_id: data.service_execution_id || "",
  ...
  function update(name, value) {
    ...
    if (name === "service_execution_id" && value) {
  ...
  async function applyOrderToInvoice(orderId) {
    try {
      const resp = await api.get(`/purchase/service-executions/${orderId}`);
  ...
  async function submit(e) {
    ...
    try {
      ...
      const payload = {
        ...
        ...(form.service_execution_id
          ? { service_execution_id: Number(form.service_execution_id) }
          : {}),
      ...
      try {
        ...
        if (autoDelivery) {
          try {
            ...
            const dPayload = {
              ...
              customer_id: Number(form.customer_id),
              service_execution_id: form.service_execution_id
                ? Number(form.service_execution_id)
                : null,
        ...
        navigate("/service-management/service-invoices");
      } catch (subErr) {
        const smsg =
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => update("service_execution_id", e.target.value)}
                  disabled={readOnly}
                >
