...
async function ensurePMMaterialRequisitionTables(companyId, branchId) {
  ...
  try { await query(`ALTER TABLE pm_material_requisitions ADD COLUMN IF NOT EXISTS is_active ENUM('Y','N') NOT NULL DEFAULT 'Y'`); } catch (e) {}
  try { await query(`ALTER TABLE pm_material_requisitions ADD COLUMN IF NOT EXISTS deleted_at DATETIME NULL`); } catch (e) {}
}

async function nextPMMRNo(companyId, branchId) {
  const rows = await query(`SELECT requisition_no FROM pm_material_requisitions WHERE company_id = :companyId AND branch_id = :branchId AND requisition_no LIKE 'PMR-%' ORDER BY CAST(SUBSTRING(requisition_no, 5) AS UNSIGNED) DESC LIMIT 1`, { companyId, branchId });
  let nextNum = 1;
  if (rows.length > 0) {
  ...
  return `PMR-${String(nextNum).padStart(6, "0")}`;
}
...
async function nextPMUtilNo(companyId, branchId) {
  const rows = await query(`SELECT utilization_no FROM pm_material_utilization WHERE company_id = :companyId AND branch_id = :branchId AND utilization_no LIKE 'UTL-%' ORDER BY CAST(SUBSTRING(utilization_no, 5) AS UNSIGNED) DESC LIMIT 1`, { companyId, branchId });
  let nextNum = 1;
  if (rows.length > 0) {
  ...
  return `UTL-${String(nextNum).padStart(6, "0")}`;
}
...
export const listPMMaterialRequisitions = async (req, res, next) => {
  try {
    ...
    const rows = await query(`SELECT r.*, p.project_name, w.warehouse_name, d.dept_name AS department_name, u.username AS created_by_name
...
export const getPMMaterialRequisitionById = async (req, res, next) => {
  try {
    ...
    if (!id) throw httpError(400, "VALIDATION_ERROR", "Invalid id");
    await ensurePMMaterialRequisitionTables(companyId, branchId);
    const [hdr] = await query(`SELECT r.*, p.project_name, w.warehouse_name, d.dept_name AS department_name, u.username AS created_by_name
      FROM pm_material_requisitions r
      LEFT JOIN pm_projects p ON r.project_id = p.id
      LEFT JOIN inv_warehouses w ON r.warehouse_id = w.id
      LEFT JOIN hr_departments d ON r.department_id = d.id
...
export const createPMMaterialRequisition = async (req, res, next) => {
  ...
  try {
    ...
    await ensurePMMaterialRequisitionTables(companyId, branchId);
    const b = req.body || {};
    const reqNo = b.requisition_no || await nextPMMRNo(companyId, branchId);
...
export const listPMMaterialUtilizations = async (req, res, next) => {
  try {
    ...
    const rows = await query(`SELECT u.*, p.project_name, t.task_title, u2.username AS created_by_name
...
export const createPMMaterialUtilization = async (req, res, next) => {
  ...
  try {
    ...
    const details = Array.isArray(b.details) ? b.details : [];
    await conn.beginTransaction();
    const [hdr] = await conn.execute(`INSERT INTO pm_material_utilization (company_id, branch_id, utilization_no, utilization_date, project_id, task_id, task_summary, location, remarks, status, created_by)
      VALUES (:companyId, :branchId, :utilNo, :utilDate, :projectId, :taskId, :taskSummary, :location, :remarks, :status, :createdBy)`, {
...
export const updatePMMaterialUtilization = async (req, res, next) => {
  ...
  try {
    ...
    const details = Array.isArray(b.details) ? b.details : [];
    await conn.beginTransaction();
    await conn.execute(`UPDATE pm_material_utilization SET utilization_date = :utilDate, project_id = :projectId, task_id = :taskId, task_summary = :taskSummary, location = :location, remarks = :remarks, status = :status WHERE id = :id AND company_id = :companyId AND branch_id = :branchId`, {
      id, companyId, branchId,
  ...
  try {
    ...
    for (const d of details) {
      ...
      await conn.execute(`INSERT INTO pm_material_receipt_items (receipt_id, item_id, item_name, uom, transfer_qty, receipt_qty, batch_no, expiry_date, mfg_date) VALUES (:rcptId, :itemId, :itemName, :uom, :transferQty, :receiptQty, :batchNo, :expiryDate, :mfgDate)`, {
        ...
        receiptQty: Number(d.receipt_qty || 0),
        batchNo: d.batch_no || null,
...
export const getPendingIssueToRequirement = async (req, res, next) => {
  try {
    const { companyId, branchId } = req.scope;
    await ensurePMMaterialReceiptTables(companyId, branchId);
    const rows = await query(`SELECT i.id, i.issue_no, i.issue_date, i.warehouse_id, i.department_id, i.issued_to, i.status, i.remarks,
      ...
      LEFT JOIN hr_departments d ON i.department_id = d.id
      WHERE i.company_id = :companyId AND i.branch_id = :branchId
      AND (LOWER(d.dept_name) LIKE '%project%management%' OR i.department_id IN (SELECT id FROM hr_departments WHERE company_id = :companyId2 AND LOWER(dept_name) LIKE '%project%management%'))
      AND i.status IN ('POSTED', 'ISSUED')
      AND i.id NOT IN (SELECT issue_id FROM pm_material_receipts WHERE issue_id IS NOT NULL)
      ORDER BY i.issue_date DESC`, { companyId, branchId, companyId2: companyId });
    res.json({ items: rows });
  } catch (err) { next(err); }
};
...
export const getIssueToRequirementDetail = async (req, res, next) => {
  try {
    ...
    const [hdr] = await query(`SELECT i.*, w.warehouse_name, d.dept_name AS department_name
      FROM inv_issue_to_requirement i
      LEFT JOIN inv_warehouses w ON i.warehouse_id = w.id
      LEFT JOIN hr_departments d ON i.department_id = d.id
...
export const getProjectStatusReport = async (req, res, next) => {
  try {
    ...
    const rows = await query(`SELECT p.id, p.project_code, p.project_name, p.project_status, p.completion_percent,
      ...
      WHERE p.company_id = :companyId AND p.branch_id = :branchId
      GROUP BY p.id
      ORDER BY p.start_date DESC`, { companyId, branchId });
    res.json({ items: rows });
  } catch (err) { next(err); }
};
