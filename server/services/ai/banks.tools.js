...
function hasModuleAccess(requiredModules, scope = {}) {
  ...
  const allowed = (scope.allowedModules || []).map((m) => String(m).toLowerCase());
...
export const BANKS_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get retail POS metrics: today's sales revenue, today's transactions count, 30-day trends, and top selling retail products.",
      parameters: {
        type: "object",
        properties: {},
      },
      ...
      case "get_pos_performance": {
        const [
          ...
          sq(
            `SELECT 
              i.item_name,
              SUM(poi.quantity) as totalQtySold,
              SUM(poi.total) as totalRevenue
             FROM pos_order_items poi
             LEFT JOIN inv_items i ON poi.item_id = i.id
